...
def get_rema_dem_for_bounds(
    ...
    dem_paths = download_REMA_tiles(
        s3_url_list[0:], resolution, save_folder=rema_folder_path
    )

    # note logic here should crop to bounds simillar to cop30 logic
    print("combining DEMS")
    merge(sources=dem_paths, dst_path=save_path, bounds=bounds_poly_3031.bounds)
